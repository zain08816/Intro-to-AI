#! /usr/bin/python3
'''
The above line will tell the bash/zsh shell to run the following code
through the python3 interpreter. This is useful if you wish to make this
script executable, to simply run by typing "$ ./grid-world-generator.py"

You may have to install numpy, matplotlib, and ipython before running this
script.

FYI - I'm getting some errors that appear to come from somewhere deep in I don't
know what module/library:
"... Error calling StartServiceByName for org.gtk.vfs.Daemon: ...". 
The code still seems to generate the mazes and plot a .png for each. \shrug
\s MBS
'''
import sys
import numpy as np
import random as rr
##  added the following condition to account for matplotlib bug in macOS/OSX.
#   see Announcement on Sakai. \s MBS
if sys.platform == 'darwin': # 'darwin' is sys.platform value for macOS
    import matplotlib
    matplotlib.use('PS')
